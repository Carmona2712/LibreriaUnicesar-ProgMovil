...
public class AdministradorJpaController implements Serializable {
    ...
    public void create(Administrador administrador) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            if (findAdministrador(administrador.getUsuario()) != null) {
    ...
    public void edit(Administrador administrador) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                String id = administrador.getUsuario();
    ...
    public int getAdministradorCount() {
        ...
        public List<Administrador> getAdminCredencial(String user,String pass) {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createNativeQuery("SELECT `usuario`, `pass`, `email` FROM `administrador` WHERE `usuario` = '"+user+"' and `pass` = '"+pass+"'", Administrador.class);
            List<Administrador> lp = q.getResultList();
            return lp;
        } finally {
            em.close();
        }
